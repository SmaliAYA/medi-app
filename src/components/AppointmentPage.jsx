...
export default function AppointmentPage({ onBack, onConfirm }) {
  ...
  return (
    <div className="h-screen w-full max-w-md mx-auto bg-white flex flex-col">
      ...
      <p className="px-6 text-sm text-gray-500 mb-4">Choose the date and time that works best for you.</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6 flex flex-col gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5">
          ...
          <div className="flex gap-4 mt-3">
            <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-blue-600 inline-block" /><span className="text-xs text-gray-500">Available</span></div>
            <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-gray-300 inline-block" /><span className="text-xs text-gray-500">Unavailable</span></div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-2xl p-4 flex flex-col gap-2">
          ...
          <div className="flex justify-between"><span className="text-sm text-gray-500">Doctor</span><span className="text-sm font-medium">Dr. Youssef EL-Amrani</span></div>
          <div className="flex justify-between"><span className="text-sm text-gray-500">Date</span><span className="text-sm font-medium">{selDay ? `${selDay.d} ${MONTHS[selDay.m]} ${selDay.y}` : "—"}</span></div>
          <div className="flex justify-between"><span className="text-sm text-gray-500">Time</span><span className="text-sm font-medium">{selTime || "—"}</span></div>
        </div>

        {/* Buttons */}
        <div className="grid grid-cols-2 gap-3">
          <button onClick={onBack} className="py-4 rounded-2xl border border-gray-200 text-sm font-medium text-gray-800">Cancel</button>
          <button
